...
export default function Dashboard() {
    const { stats, chartData, recentOrders, recentOrdersTotal } = usePage<VendorDashboardProps>().props
            ...
            <div className="space-y-6 text-white p-6 min-h-screen">
                ...
                <div className="bg-bg-gray border border-gray-800/50 p-6 rounded-xl">
                    ...
                    <div className="flex gap-3 min-h-[240px]">
                        {/* Y-axis labels (50K at top, 0 at bottom) */}
                        <div
                            className="flex flex-col justify-between text-[10px] text-gray-500 shrink-0"
                            style={{ width: Y_AXIS_WIDTH }}
                        >
                            {[...yTicks].reverse().map((tick) => (
                                <span key={tick}>{tick === 0 ? '0' : formatYLabel(tick)}</span>
                            ))}
                        </div>
                        ...
                        <div className="flex-1 relative min-w-0">
                            ...
                            >
                                ...
                                {chartData.map((_, i) => (
                                    <rect
                                        key={i}
                                        x={(i / (chartData.length - 1 || 1)) * chartInnerWidth - chartInnerWidth / (chartData.length * 2)}
                                        y={0}
                            ...
                            {/* Tooltip + vertical dashed line when hovering */}
                            {hoveredPoint != null && (
                                <>
                                    <div
                                    ...
                                    >
                                        <div className="bg-[#0F1012] border border-gray-800 rounded shadow-lg overflow-hidden text-center min-w-[72px]">
                                            ...
                                            <div className="px-3 py-1.5 text-xs font-bold text-white border-t border-gray-800">
                                                ${hoveredPoint.value.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
                                            </div>
                                        </div>
                                    </div>
                                </>
                                            ...
                                            <Link
                                                href={
                                                    order.statusValue === 'cancelled'
                                                        ? route('vendor.order.cancelled-details', {
